...
export async function getAll(req, res, next) {
  try {
    const result = await extinguisherService.getAll(req.query);
...
export async function getById(req, res, next) {
  try {
    const data = await extinguisherService.getById(req.params.id);
...
export async function update(req, res, next) {
  try {
    const data = await extinguisherService.update(req.params.id, req.body);
    res.json({ success: true, data });
  } catch (error) {
    next(error);
  }
