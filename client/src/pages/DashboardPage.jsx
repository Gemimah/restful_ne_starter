...
function kpiCardsForRole(role, stats) {
  ...
  const all = [
    { title: 'Total Extinguishers', value: inv?.total ?? '—', color: 'bg-red-50 text-red-700 border-red-100' },
    { title: 'Pending Inspections', value: insp?.pending ?? '—', color: 'bg-amber-50 text-amber-700 border-amber-100' },
    { title: 'Overdue Inspections', value: insp?.overdue ?? '—', color: 'bg-orange-50 text-orange-700 border-orange-100' },
    {
      title: 'Compliance',
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-slate-600">
        Welcome, {user?.firstName || user?.name || user?.email}! Role:{' '}
